...
import activity.sostv.com.ui.FullScreenVideoActivity;
import io.bxbxbai.common.T;
import io.vov.vitamio.widget.MediaController;
import io.vov.vitamio.widget.VideoView;


public class SostvMediaController extends MediaController {
		...
		public void onClick(View v) {
			if (activity != null) {
				if ((System.currentTimeMillis() - mExitTime) > 2000) {
					T.showToast("再按一次离开视频");
					mExitTime = System.currentTimeMillis();
				} else {
					FullScreenVideoActivity.editSeekTime(videoView.getCurrentPosition());
					activity.finish();
				}
			}
		}
	...
	public SostvMediaController(Context context, VideoView videoView, Activity activity) {
		super(context);
	...
	protected View makeControllerView() {
		...
		textViewTitle = (TextView)v.findViewById(R.id.mediacontroller_file_name);
		...
		// mid
		mVolumeBrightnessLayout = (RelativeLayout) v
	...
	private class SostvGestureListener extends
	...
	public void setBattery(String stringBattery) {
		if (textViewTime != null && img_Battery != null) {
			...
			if (battery < 15)
				img_Battery.setImageDrawable(getResources().getDrawable(
						R.mipmap.battery_15));
